...
struct
  ...
  (* If a card in my hand is known to be playable, play the newest *)
  fun playPlayableCard (bs : bool list) : action option =
    Option.map Play (Util.findIndex (fn b => b) bs)
  ...
  val otherCluedValue : int = 0 (* this should be split up: usually positive (~20),
  ...
  val canBeSaveValue : int = ~200 (* higher if new chop is not important *)
  val makeCardPlayable : int = 30 (* higher all visible copies of next card are unclued on chop *)
  ...
  in
    ...
    (* print (memoryToString (!m) ^"\n"); *)
    (case playPlayableCard (hd (#playable (!m))) of
         NONE => NONE
       | SOME a => SOME (giveSaveHint s otherwise (fn () => a))) otherwise (fn () =>
